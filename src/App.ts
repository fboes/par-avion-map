...
export default class App {
  ...
  constructor() {
    this.elements = {
      ...
      airportsCanvases: <NodeListOf<HTMLCanvasElement>>(
        document.querySelectorAll(".approaches canvas")
      ),
      seedInput: <HTMLInputElement>document.getElementById("seed"),
      mapDimensionInput: <HTMLInputElement>(
        document.getElementById("mapdimension")
      ),
  ...
  generateMap() {
    (this.randomizer = new Randomizer(
      this.elements.seedInput ? this.elements.seedInput.value : "",
    )),
    ...
    this.map = new LocationsMap(
      this.elements.mapDimensionInput
  ...
  drawMap() {
    ...
    this.plane.hsi.headingSelect = new Degree(
      this.map.airports[0].approachPoints[1].coordinates.getBearing(
        this.map.airports[1].approachPoints[0].coordinates,
      ),
    );
    this.plane.heading = new Degree(
      this.map.airports[0].runways[0].heading.oppositeDegree,
    );
    this.plane.navRadios.forEach((navRadio, index) => {
      ...
      if (navRadio.bearing && navRadio.navAids[index]) {
        navRadio.setCourse(
          this.map.airports[0].coordinates.getBearing(
            navRadio.navAids[index].coordinates,
          ),
        );
      }
    });
    ...
    this.mapHsi = new CanvasMapLog(
      this.elements.mapHsiCanvas,
      this.plane,
      this.map,
    );
  ...
  onClickPosition(event: MouseEvent) {
    ...
    this.plane.hsi.headingSelect = new Degree(
      this.plane.coordinates.getBearing(clickCoordinates),
    );
      ...
      if (this.gamepad) {
        ...
        if (gamepad !== null) {
          ...
          gamepad.buttons[XboxGamepad.BUTTON_INDEX.lTrigger].triggered &&
            ...
            this.plane.hsi.navRadios[
              this.plane.hsi.activeElement === Hsi.INTERACTIVE_NAV1_SOURCE
                ? 0
                : 1
            ].changeCurrentNavaid(-1, this.plane.coordinates);
          ...
          if (
            gamepad.buttons[XboxGamepad.BUTTON_INDEX.lTrigger].value +
        ...
        const unBroken = this.plane.move(
          delta,
          this.weather.at(this.plane.coordinates),
          this.terrain.getElevationNm(this.plane.coordinates),
        );
        if (!unBroken) {
