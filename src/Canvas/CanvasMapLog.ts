...
export default class CanvasMapLog {
  ...
  draw() {
    ...
    if (this.showLog) {
      this.plane.flightLog.coordinates.forEach((c) => {
        let t = new CanvasTool(
          this.ctx,
          c.coordinates.x * this.multiplier,
          c.coordinates.y * this.multiplier,
          1,
        );
        ...
        t.lineRaw(
          ...
          3,
          0,
          c.altAglFt ? -c.altAglFt * altitudeMulti : 0,
        ).stroke();
    ...
    t = this.getCanvasTool(
      this.plane.altAglFt ? -this.plane.altAglFt * altitudeMulti : -2,
    );
    t.rotate(0, 0, this.plane.hsi.heading.degree);
  ...
  getAlpha(): number {
    ...
    this.map.airports.forEach((airport) => {
      distance = Math.min(
        distance,
        airport.coordinates.getDistance(this.plane.coordinates),
      );
    });
  ...
  getCanvasTool(offset = 0) {
    return new CanvasTool(
      ...
      this.plane.coordinates.y * this.multiplier + offset,
      1,
    );
  }
}
