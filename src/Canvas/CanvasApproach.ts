...
export default class CanvasApproach {
  ...
  makeInfoBox() {
    ...
    const lineHeight = 7;
    const ilsFrequency = this.airport.runways[0].ils.first
      ? this.airport.runways[0].ils.first.frequency
      : null;
    const rows = {
    ...
    t.text(
      1,
      lineHeight * 1,
      this.airport.name + " (" + this.airport.code + ")",
    );
    ...
    t.text(
      ...
      (ilsFrequency ? "ILS " : "") +
        "RWY " +
        CanvasTool.numPad(
          Math.round(this.airport.runways[0].heading.oppositeDegree / 10),
          2,
        ),
    );
    ...
    t.text(
      rows.firstCenter,
      3 + lineHeight * 2,
      this.airport.hasTower ? "CT" : "UNICOM",
    );
    ...
    t.text(
      rows.firstCenter,
      3 + lineHeight * 3,
      CanvasTool.frequency(this.airport.frequency),
    );
    t.text(
      rows.secondCenter,
      3 + lineHeight * 3,
      this.airport.runways[0].heading.oppositeDegree.toFixed() + "°",
    );
    if (this.airport.coordinates.elevation) {
      t.text(
        rows.thirdCenter,
        3 + lineHeight * 3,
        this.airport.coordinates.elevation.toFixed(),
      );
    }
    if (ilsFrequency) {
      t.text(
        rows.fourthCenter,
        3 + lineHeight * 3,
        CanvasTool.frequency(ilsFrequency),
      );
    }
    ...
    t.text(
      this.maxX - 3,
      this.maxY - 3,
      CanvasTool.frequency(this.airport.coordinates.x) +
        "X | " +
        CanvasTool.frequency(this.airport.coordinates.y) +
        "Y",
    );
  ...
  makeAirport() {
    ...
    const xCenter =
      this.maxX / 2 + Math.cos(rad) * (600 / CanvasApproach.FACTOR);
    const yCenter =
      (this.maxY - 27) / 2 + 27 + Math.sin(rad) * (600 / CanvasApproach.FACTOR);
    ...
    if (this.airport.hasTower) {
      const t2 = this.getNewCanvasTool(
        this.maxX / 2,
        (this.maxY - 27) / 2 + 27,
      );
  ...
  makeRunways() {
    ...
    this.navaids.forEach((navaid) => {
      ...
      t.text(
        ...
        navaid.code +
          ...
          bearing.toFixed() +
          "° " +
          navaid.coordinates.getDistance(this.airport.coordinates).toFixed(2) +
          "NM",
      );
      t.reset();
    });
    ...
    this.airport.runways.forEach((runway) => {
      ...
      t.polygonRaw([
        ...
        [
          runwayX / 2.1 - 100 / CanvasApproach.FACTOR,
      ...
      [0, 1].forEach((i) => {
        ...
        const deg =
          i === 0 ? runway.heading.oppositeDegree : runway.heading.degree;
        ...
        t.text(
          0,
          posY + 4,
          CanvasTool.numPad(Math.round(deg / 10), 2),
          CanvasApproach.OUTLINE,
        );
        t.rotate(0, 0, multiplier * 90);
      });
      ...
      t.text(
        0,
        invers ? 5.5 : -2.5,
        runway.length.toFixed() + " × " + runway.width.toFixed(),
        CanvasApproach.OUTLINE,
      );
  ...
  protected isShortApproach(approachLight: string) {
    return (
      approachLight === Runway.ODALS ||
      approachLight === Runway.SALS ||
      approachLight === Runway.MALS
    );
  }
  ...
  protected makeLight(x: number, y: number, t: CanvasTool, label: string) {
    ...
    t.textStyle(4);
    t.style(
      pilotControlledLight ? this.colors.black : this.colors.white,
      pilotControlledLight ? this.colors.white : this.colors.black,
      0.5,
    );
    t.circle(x, y, 3).fill();
    this.ctx.stroke();
  ...
  protected makeApproach(x: number, y: number, t: CanvasTool, label: string) {
    ...
    for (
      let i = 0;
      i <= (this.isShortApproach(label) ? 1500 : 2500);
      i += 250
    ) {
      let width = 100;
      if (i === 0) {
      ...
      if (width > 0) {
        if (
          (i > 1500 && (label === Runway.MALSR || label === Runway.SSALR)) ||
          label === Runway.ODALS
        ) {
          t.line(
            x,
            y + i / CanvasApproach.FACTOR,
            x,
            y + (i + width) / CanvasApproach.FACTOR,
          ).stroke();
        } else {
          t.line(
            x - width / 2 / CanvasApproach.FACTOR,
            y + i / CanvasApproach.FACTOR,
            x + width / 2 / CanvasApproach.FACTOR,
            y + i / CanvasApproach.FACTOR,
          ).stroke();
      ...
      if (i > 0 && i < 1000 && label === Runway.ALSF2) {
        let width2 = 50;
        t.line(
          x - (width + 300) / 2 / CanvasApproach.FACTOR,
          y + i / CanvasApproach.FACTOR,
          x - (width + 300 - width2) / 2 / CanvasApproach.FACTOR,
          y + i / CanvasApproach.FACTOR,
        ).stroke();
        t.line(
          x + (width + 300) / 2 / CanvasApproach.FACTOR,
