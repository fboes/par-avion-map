...
export default class CanvasMapHsi {
  ...
  protected drawNavRadio(navRadio: NavRadio, index: number) {
    ...
    if (navRadio.distance) {
      this.text(
        ...
        index === 0 ? -125 : +125,
        115,
        navRadio.distance.toFixed(2) + " NM",
        "DME",
        align,
      );
    }
    if (navRadio.course) {
      this.text(
        t,
        index === 0 ? -125 : +125,
        navRadio.distance ? 83 : 115,
        navRadio.course.degree.toFixed(0).padStart(3, "0") + "°",
        "CRS",
        align,
      );
      ...
      this.text(
        ...
        index === 0 ? -125 : +125,
        navRadio.distance ? 83 : 115,
        navRadio.bearing.degree.toFixed(0).padStart(3, "0") + "°",
        "BRG",
        align,
      );
    }
    ...
    if (navRadio.course) {
      ...
      if (navRadio.deviation) {
        ...
        deviation = Math.max(
          -maxDeviationDegrees,
          Math.min(maxDeviationDegrees, deviation),
        );

        const currentDeviation =
          (deviation / -maxDeviationDegrees) * deviationMaxRangePx;
        ...
        if (navRadio.distance) {
          const distance =
            (Math.min(10, navRadio.distance) / 10) *
            (circleDeviationPx - 3) *
            (isTo ? 1 : -1);
          t.line(
            currentDeviation - 9,
    ...
    if (
      ...
      if (navRadio.distance && navRadio.type === Navaid.NDB) {
        const distance =
          (Math.min(10, navRadio.distance) / 10) * (maxArrow - 15);
  ...
  getCanvasTool() {
    return new CanvasTool(
      this.ctx,
      this.plane.coordinates.x * this.multiplier,
      this.plane.coordinates.y * this.multiplier,
      1,
    );
  }
