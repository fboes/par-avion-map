...
export default class LocationsMap {
  ...
  public constructor(
    ...
    this.mapDimension = Math.max(
      6 + LocationsMap.PADDING * 2,
      Math.ceil(mapDimension / 2) * 2,
    );
  ...
  protected rand(deviation = 30) {
    ...
    this.airports[0].addRunway(
      this.randomizer.getDegree(this.windDirection.degree, deviation),
    );

    const destinationDegree = this.randomizer.getDegree(
      departureDegree.oppositeDegree,
      deviation * 2,
    );
    ...
    if (this.mapDimension >= 10 + 2 * LocationsMap.PADDING) {
      this.navAids[0].randHoldingPattern(
        this.navAids[0].coordinates.getBearing(
          this.airports[1].approachPoints[0].coordinates,
        ),
      );
    }
          ...
          new Obstruction(
            this.getCoordinates(
              this.randomizer.getInt(
                0,
                allowedRadius + LocationsMap.PADDING / 2,
              ),
              this.randomizer.getDegree(departureDegree.degree + 90, 70),
            ),
            this.randomizer,
    ...
    if (this.mapDimension >= 20 + 2 * LocationsMap.PADDING) {
      this.navAids[1].randHoldingPattern(
        this.navAids[1].coordinates.getBearing(
          this.airports[0].approachPoints[0].coordinates,
        ),
      );
      const destinationDegree2 = this.randomizer.getDegreeBetween(
        departureDegree,
        destinationDegree,
      );
  ...
  public getHighestObstruction(
    ...
    this.obstructions.forEach((obstruction) => {
      if (
        ...
        let totalHeight =
          obstruction.coordinates.elevation + obstruction.heightAboveGround;
